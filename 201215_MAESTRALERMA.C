...
*******************************************************************************/
	union {                                                                       // UNION conversion de tipo datos
	      float          conver_float;
...
*******************************************************************************/
	...
	{
	   if(good==0x01){                                                                            // COMUNICACION EXITOSA
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	unsigned int calcula_estado_enlaceweb(unsigned int num){
		if(enlaceweb_cont[num]==my4XRegs[tabla[num].enlace_hmi + 2300]){// no ha camabiado el contador
			if(read_rtc() - enlaceweb_timerweb[num] > TIMEOUT_ESTADOWEB || flag_inicio==1){// ya pasó el timeout
				flag_inicio=0;
	...
	cofunc void COF_captura_datos_scairlink(unsigned int No_dispo)
	{
	   ...
	   if( error == 0xffff ){
	      ...
	      if( my4XRegs[tabla[No_dispo].enlace_hmi] == 0x01 ){// si hay enlace Rf fecha se obtiente con almacena datos tiempo
	          apunta_tiempo = (int *)&my4XRegs[tabla[No_dispo].tiempo_hmi];
	          Almacena_Datos_Tiempo( apunta_tiempo );
	          calcula_estado_enlaceweb(No_dispo);

	          if(enlaceweb_estado[No_dispo]==0x01){//rf ok + web ok
	      	...
	      	if(enlaceweb_estado[No_dispo]==0x01){//rf mal + web ok
	      		apunta_tiempo = (int *)&my4XRegs[tabla[No_dispo].tiempo_hmi];
	          Almacena_Datos_Tiempo( apunta_tiempo );
    ...
    costate{
      if(new_seg!=read_rtc()){                                             // New Second ?
        ...
        if(new_hora != rtc.tm_hour){
          ...
          my4XRegs[U_año]            = rtc.tm_year + 1900;
        ...
        if( conta_seg >= 2 ){
          ...
          for(zz=0; zz < No_max_sitios; zz++){
            if( zz != dispo_local){
            	PRINTFDEBUG("RD slave [%d]\n",zz);
           		wfd COF_captura_datos_scairlink(zz);
            }
          }
          ...
          */
          ...
          if( my4XRegs[reset_slink] == 0xaa55 ){													// Reset hardware Medidor de Nivel
           	digOut(0, 0);
